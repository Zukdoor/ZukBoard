...
NOW_DEPLOY_ID=$( now --no-clipboard --regions=all --docker --token=$NOW_TOKEN -e OSS_ID=$OSS_ID -e OSS_SECRET=$OSS_SECRET -e MONGODB_URI=$MONGODB_URI_NOW_STAGING -e GA_ID=$GA_ID_NOW_STAGING)
