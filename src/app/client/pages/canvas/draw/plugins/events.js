      ...
      if (data.type && data.listener) {
        ...
        for (let key in data) {
          ...
          if (typeof (o) === 'function') { // without configuration
            ...
            listener = o.listener
            for (let k in o) { // merge configure into base configuration
    ...
    for (var i = 0; i < length; i++) {
      ...
      if (conf.fingers) {
        if (conf.fingers >= conf.maxFingers) {
          var ids = []
          for (let sid in conf.tracker) ids.push(sid)
...
eventjs.proxy = (function (root) {
  ...
  root.gesture = function (conf) {
    ...
    conf.onPointerMove = function (event, state) {
      ...
      for (let sid in points) {
        let touch = points[sid]
        if (touch.up) continue
        ...
        // Calculate scale.
        dx = touch.move.x - self.x
        dy = touch.move.y - self.y
    ...
    conf.onPointerUp = function (event) {
      if (root.pointerEnd(event, self, conf)) {
        ...
        for (var sid in conf.tracker) {
          ...
          degree2 = Math.atan2(xdist, ydist) / RAD_DEG + 180
          velocity2 = ms ? distance / ms : 0
